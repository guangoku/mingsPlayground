    ...
    <section className="relative min-h-screen flex items-center overflow-hidden hero-bg">
      ...
      <div className="relative z-10 w-2/5 ml-auto flex flex-col justify-center items-end pr-12 min-h-screen">
        <div className="text-right">
          ...
          <h1 className="text-5xl md:text-7xl font-bold mb-2" style={{ color: 'hsl(var(--coral-fixed))', textShadow: '2px 2px 4px rgba(0,0,0,0.3)', fontFamily: 'Luckiest Guy, cursive' }} data-testid="text-hero-name">
            {language === 'en' ? "Ming's" : '关关的'}
          </h1>
          <h1 className="text-5xl md:text-7xl font-bold mb-8" style={{ color: 'hsl(var(--coral-fixed))', textShadow: '2px 2px 4px rgba(0,0,0,0.3)', fontFamily: 'Luckiest Guy, cursive' }} data-testid="text-hero-playground">
            {language === 'en' ? 'Playground' : '游乐场'}
          </h1>

          <h2 className="text-2xl md:text-3xl font-bold mb-4" style={{ color: 'hsl(var(--foam-fixed))', textShadow: '1px 1px 2px rgba(0,0,0,0.3)' }} data-testid="text-hero-subtitle1">
            {language === 'en' ? 'Innovating with Data' : '用数据创新'}
          </h2>
          <h2 className="text-2xl md:text-3xl font-bold mb-4" style={{ color: 'hsl(var(--foam-fixed))', textShadow: '1px 1px 2px rgba(0,0,0,0.3)' }} data-testid="text-hero-subtitle2">
            {language === 'en' ? 'Creating Art' : '创造艺术'}
          </h2>
          <h2 className="text-2xl md:text-3xl font-bold mb-8" style={{ color: 'hsl(var(--foam-fixed))', textShadow: '1px 1px 2px rgba(0,0,0,0.3)' }} data-testid="text-hero-subtitle3">
